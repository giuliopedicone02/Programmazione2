...
public:
    ...
    void preorder(BSTNode<T> *ptr)
    {
        if (ptr == nullptr)
            return;
        visit(ptr);           // Visita la radice
        preorder(ptr->left);  // Visita il sottoalbero sinistro
        preorder(ptr->right); // Visita il sottoalbero destro
    }
    ...
    void inorder(BSTNode<T> *ptr)
    {
        ...
        inorder(ptr->left);  // Visita il sottoalbero sinistro
        visit(ptr);          // Visita la radice
        inorder(ptr->right); // Visita il sottoalbero destro
    }
    ...
    void postorder(BSTNode<T> *ptr)
    {
        ...
        postorder(ptr->left);  // Visita il sottoalbero sinistro
        postorder(ptr->right); // Visita il sottoalbero destro
        visit(ptr);            // Visita la radice
    }
    ...
    void deleteUnder(double area)
    {
        BSTNode<T> *aux = getRoot();

        while (aux)
        {
            std::cout << *(aux->getKey()) << std::endl;
            //  std::cout << aux->getKey() << std::endl;
            if (aux->getKey()->getArea() < area)
            {
                remove(aux->getKey());
            }

            if (!aux || !successor(aux))
            {
                return;
            }
            else
    ...
    void stampaOggetto(BSTNode<T> *aux)
    {
        std::cout << *(aux->getKey()) << std::endl;
    }

    void visualizza(BSTNode<T> *ptr)
    {
        if (ptr == nullptr)
            return;
